...
class GitHubClientTest < Minitest::Test
  ...
  def test_create_release_writes_a_draft_release
    shell = FakeShell.new(
      {
        ["gh", "release", "create", "v7.3.0", "--repo", "puma/puma", "--title", "v7.3.0", "--notes-file", "/tmp/NOTES", "--draft"] => FakeShell::Result.new(stdout: "", stderr: "", success?: true, exitstatus: 0),
        ["gh", "release", "view", "v7.3.0", "--repo", "puma/puma", "--json", "tagName,name,isDraft,body,url,assets,targetCommitish"] => FakeShell::Result.new(stdout: '{"tagName":"v7.3.0","isDraft":true}', stderr: "", success?: true, exitstatus: 0)
      }
    )
    ...
    client = PumaRelease::GitHubClient.new(context)
    client.define_singleton_method(:with_notes_file) { |_body, &block| block.call("/tmp/NOTES") }
    release = client.create_release("v7.3.0", "notes", title: "v7.3.0", draft: true)

    assert_equal [["gh", "release", "create", "v7.3.0", "--repo", "puma/puma", "--title", "v7.3.0", "--notes-file", "/tmp/NOTES", "--draft"]], confirmations
    assert_equal true, release.fetch("isDraft")
  end
end
